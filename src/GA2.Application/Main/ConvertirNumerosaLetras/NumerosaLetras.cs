...
namespace GA2.Application.Main
{
    ...
    
    {
        ...
        public string GetDayInLetters(DateTime date)
        {
            return this.Convertir(date.Day.ToString());
        }
        public string GetMonthInLetters(DateTime date)
        {
            switch (date.Month)
            {
                case 1:
        ...
        public string GetYearInLetters(DateTime date)
        {
            var response = this.Convertir(date.Year.ToString());
            var array = date.Year.ToString().ToCharArray();
            if (array[array.Length-1]=='1')
            {
                response = response.Replace("UN", "UNO");
            }
            return response;
        }
