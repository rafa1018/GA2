...
namespace GA2Functions.Endpoints.Credito
{
    ...
    public class EliminarTipoActividadPorId
    {
        ...
        public EliminarTipoActividadPorId(ICreditoBusinessLogic creditoBusinessLogic, ITokenClaims tokenClaims)
        {
            _creditoBusinessLogic = creditoBusinessLogic;
            _tokenClaims = tokenClaims;
        }

        [Function("EliminarTipoActividadPorId")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.User, "delete", Route = "eliminarTipoActividadPorId/{id}")]
            HttpRequestData req, string id, FunctionContext executionContext)
        {
            ...
            var user = claims;
            var result = await _creditoBusinessLogic.EliminarTipoActividadPorid(id);
